...
class Qlearner:
    ...
    def initializeQtable(self, env):
        self.Q = tf.nn.Sequential(
            tf.nn.Linear(env.observation_space.n),
            tf.nn.ReLU(),
            tf.nn.Linear(128),
            tf.nn.ReLU(),
            tf.nn.Linear(env.action_space.n)
        )
    ...
    def train(self, num_episodes, max_steps):
        for i in range(num_episodes):
            ...
            for step in range(max_steps):
                ...
                self.Q = self.Q[state][action] + self.a * (reward + self.g * np.max(self.Q(next_state)) - self.Q[state][action])
                if done:
                    break
                state = next_state
